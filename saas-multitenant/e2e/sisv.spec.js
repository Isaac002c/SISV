...
test.describe.serial('SISV 1.0 - fluxos críticos', () => {
  test('administrador cria usuário operacional e cliente fictício', async ({ page }) => {
    ...
    await expect(clientDialog.locator('form').locator('input, select, textarea').first())
      .toHaveAttribute('id', 'client-category');
    await expect(clientDialog.getByLabel(/Tipo de cliente/)).toHaveCount(0);
    await expect(clientDialog.getByLabel(/^Status/)).toHaveCount(0);
    ...
    await expect(clientDialog.getByLabel(/Nome completo/)).toHaveValue(createdClient);
    ...
    await expect(clientDialog.getByLabel(/Origem do cliente/).locator('option[value="carteira"]')).toHaveCount(0);
    await expect(clientDialog.getByLabel(/Origem do cliente/).locator('option[value="campanha"]')).toHaveCount(1);
    ...
    await clientDialog.getByRole('button', { name: 'Mostrar senha' }).first().click();
    await expect(clientDialog.locator('#client-detran-password')).toHaveAttribute('type', 'text');
